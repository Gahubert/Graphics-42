...
/*   Updated: 2017/03/09 21:12:08 by ghubert          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		map_trace(t_fdf *stk)
{
	...
	if (stk->z_p)
		free(stk->z_p);
	stock_create(stk);
	if (stk->xmax == 1 && stk->ymax == 1)
	{
		point = get_center(stk);
		stk->x1 = stk->x_p[0] * stk->zo + point + stk->calx;
		stk->y1 = stk->y_p[0] * stk->zo + point + stk->calx;
		*(unsigned *)(stk->gda + ((int)stk->x1 * stk->szl) +
				((int)stk->y1 * stk->bpx / 8)) = stk->color;
	}
	else
	{
		trace_vertical(stk);
